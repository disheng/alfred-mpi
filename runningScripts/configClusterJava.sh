...
MACHINEFILE=`pwd`/$MACHINEFILENAME
...
then
	...
	cat $MACHINEFILE | while read line; do 
	    ...
	    ssh $line "apt-get update && apt-get install openjdk-7-jdk -y"
	    ssh $line "update-alternatives --set java /usr/lib/jvm/java-7-openjdk-amd64/jre/bin/java"
	done
else
